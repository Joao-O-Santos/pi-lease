...
async function dirs() {
	const root = await mkdtemp(path.join(tmpdir(), "lease-browser-"));
	return { profileDir: path.join(root, "profile"), runtimeDir: path.join(root, "run") };
}

test("starts verified fake browser and idempotently closes it", {
...
test("rejects Chrome and lease profile conflicts", async () => {
	const d = await dirs();
	await mkdir(d.profileDir, { recursive: true });
...
test("abort during startup releases ownership", {
	skip: process.platform !== "linux",
}, async () => {
	const d = await dirs(),
		controller = new AbortController();
	controller.abort();
	await assert.rejects(startBrowser({ executable: fake, ...d, signal: controller.signal }), {
		name: "AbortError",
	});
});
