...
async function executable(file) {
  try { await access(file, 0o1); return true; } catch { return false; }
}
...
export async function discoverBrowser(override, { env = process.env, platform = process.platform } = {}) {
  if (platform !== 'linux') {
    throw new Error(`browser discovery unsupported on platform: ${platform}`);
  }
  if (override) {
    const candidate = path.resolve(override);
    if (!await executable(candidate)) throw new Error(`browser executable is not executable: ${candidate}`);
    return candidate;
  }
