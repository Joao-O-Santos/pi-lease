...
const abortError = () =>
	Object.assign(new Error("browser startup aborted"), { name: "AbortError" });
...
async function procIdentity(pid, profile, expectedStart) {
	...
	try {
		...
		return {
			ok:
				cmd.some((value) => value.includes(profileFlag)) &&
				(!expectedStart || start === expectedStart),
			pgrp,
			start,
		};
	} catch {
	...
	try {
		...
		const inodes = new Set(
			rows
				...
				.filter((x) => x[1] === `0100007F:${wanted}` && x[3] === "0A")
				.map((x) => x[9]),
		);
		...
		for (const fd of readdirSync(`/proc/${pid}/fd`)) {
			try {
				if (
					inodes.has((readlinkSync(`/proc/${pid}/fd/${fd}`).match(/^socket:\[(\d+)\]$/) || [])[1])
				)
					return true;
			} catch {}
		}
	...
	await new Promise((resolve) => {
		...
		socket.on("data", (data) => {
			if (!upgraded) {
				...
				for (let i = 0; i < body.length; i++) payload[i] = body[i] ^ mask[i % 4];
...
export async function startBrowser({
	...
	if (process.platform !== "linux")
		throw diagnostic("platform", `unsupported: ${process.platform}`);
	...
	try {
		...
		if (e.code === "EEXIST") throw diagnostic("profile", `existing lease lock: ${lock}`);
		throw e;
	}
	let child,
		stateFile,
	...
	try {
		...
		if (!startIdentity?.ok || startIdentity.pgrp !== child.pid)
			throw diagnostic("ownership", `cannot prove launched PID ${child.pid}`);
		...
		while (Date.now() < deadline) {
			...
			try {
				const portLines = (await readFile(path.join(profile, "DevToolsActivePort"), "utf8"))
					.trim()
					.split(/\r?\n/);
				const port = Number(portLines[0]),
					browserPath = portLines[1];
				if (
				...
				if (!endpoint || !identity?.ok || !(await listenerOwned(child.pid, port)))
					throw new Error("unverified endpoint");
				const targets = await getJson(`http://127.0.0.1:${port}/json/list`, deadline - Date.now());
				if (
					!Array.isArray(targets) ||
					!targets.some((x) => x.type === "page" && typeof x.webSocketDebuggerUrl === "string")
				)
		...
		await chmod(runtime, 0o700).catch(() => {});
		stateFile = path.join(runtime, `browser-${randomUUID()}.json`);
		await writeFile(
			stateFile,
			JSON.stringify({
				id: path.basename(stateFile),
				pid: child.pid,
				profile,
		...
		async function close() {
			...
			for (const sig of done ? [] : ["SIGTERM", "SIGKILL"]) {
				...
				try {
					...
					exited.then(() => true),
					sleep(sig === "SIGTERM" ? 1000 : 250).then(() => false),
				]);
			}
